...
def prepare_data(data, proportion=0.96, verbose=False):
    random.shuffle(data)
...
for pp in range(100):
    for entry in train_ds:
        for i in range(inputs):
            S[i].rate = xform(float(entry[i]), MI[i], MX[i])*Hz
            print i, float(entry[i]), xform(float(entry[i]), MI[i], MX[i])
        case = int(entry[4])
        run(500*ms)
...
for entry in test_ds:
    ...
    run(500*ms)
    print AB, AB.index(max(AB)), case
    if AB.index(max(AB)) == case:
        TT += 1
    AB = [0]*outputs
print 100*TT/len(test_ds)
